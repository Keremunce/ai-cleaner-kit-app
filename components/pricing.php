<section class="pricing" id="pricing">
    <div class="container">
        <div class="section-header">
            <h2 class="section-title">Choose Your Plan</h2>
            <p class="section-description">
                Get the most out of AI Cleaner Kit with our flexible subscription plans
            </p>
        </div>
        
        <div class="pricing-grid">
            <!-- Weekly Plan -->
            <div class="pricing-card">
                <div class="pricing-header">
                    <h3 class="pricing-title">Weekly</h3>
                </div>
                <div class="pricing-price">
                    <span class="price-amount">$6.99</span>
                    <span class="price-period">/week</span>
                </div>
                    <a href="https://apps.apple.com/us/app/id6751584991" target="_blank" rel="noopener noreferrer" class="btn btn-primary pricing-btn">Start Free Trial</a>
                <p class="trial-note">3-day free trial, then $6.99/week</p>
            </div>
            
            <!-- 3D Weekly Plan -->
            <div class="pricing-card">
                <div class="pricing-header">
                    <h3 class="pricing-title">3D Weekly</h3>
                </div>
                <div class="pricing-price">
                    <span class="price-amount">$8.99</span>
                    <span class="price-period">/week</span>
                </div>
                    <a href="https://apps.apple.com/us/app/id6751584991" target="_blank" rel="noopener noreferrer" class="btn btn-primary pricing-btn">Start Free Trial</a>
                <p class="trial-note">3-day free trial, then $8.99/week</p>
            </div>
            
            <!-- Monthly Plan -->
            <div class="pricing-card featured">
                <div class="pricing-header">
                    <h3 class="pricing-title">Monthly</h3>
                    <div class="pricing-badge popular">Most Popular</div>
                </div>
                <div class="pricing-price">
                    <span class="price-amount">$14.99</span>
                    <span class="price-period">/month</span>
                </div>
                    <a href="https://apps.apple.com/us/app/id6751584991" target="_blank" rel="noopener noreferrer" class="btn btn-primary pricing-btn">Start Free Trial</a>
                <p class="trial-note">3-day free trial, then $14.99/month</p>
            </div>
            
            <!-- 3D Monthly Plan -->
            <div class="pricing-card">
                <div class="pricing-header">
                    <h3 class="pricing-title">3D Monthly</h3>
                </div>
                <div class="pricing-price">
                    <span class="price-amount">$19.99</span>
                    <span class="price-period">/month</span>
                </div>
                    <a href="https://apps.apple.com/us/app/id6751584991" target="_blank" rel="noopener noreferrer" class="btn btn-primary pricing-btn">Start Free Trial</a>
                <p class="trial-note">3-day free trial, then $19.99/month</p>
            </div>
            
            <!-- Yearly Plan -->
            <div class="pricing-card">
                <div class="pricing-header">
                    <h3 class="pricing-title">Yearly</h3>
                </div>
                <div class="pricing-price">
                    <span class="price-amount">$24.99</span>
                    <span class="price-period">/year</span>
                </div>
                    <a href="https://apps.apple.com/us/app/id6751584991" target="_blank" rel="noopener noreferrer" class="btn btn-primary pricing-btn">Start Free Trial</a>
                <p class="trial-note">3-day free trial, then $24.99/year</p>
            </div>
            
            <!-- 3D Yearly Plan -->
            <div class="pricing-card">
                <div class="pricing-header">
                    <h3 class="pricing-title">3D Yearly</h3>
                </div>
                <div class="pricing-price">
                    <span class="price-amount">$34.99</span>
                    <span class="price-period">/year</span>
                </div>
                    <a href="https://apps.apple.com/us/app/id6751584991" target="_blank" rel="noopener noreferrer" class="btn btn-primary pricing-btn">Start Free Trial</a>
                <p class="trial-note">3-day free trial, then $34.99/year</p>
            </div>
        </div>
    </div>
</section>